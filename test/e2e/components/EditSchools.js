        ...
        browser.element('css selector', '.switch-to-desktop', function (result) {
            if (result.status != -1) { //Element exists, do something
                browser.click('.switch-to-desktop')
                    .waitForElementVisible('body'); // wait for the body to be rendered…//             .click('button[type=submit]')
            .pause(1000)
...
module.exports = {
    'VIC-NAPALAN-EDIT-School': function (browser) {
        ...
        browser.element('css selector', '.switch-to-desktop', function (result) {
            if (result.status != -1) { //Element exists, do something
                browser.click('.switch-to-desktop')
                    .waitForElementVisible('body'); // wait for the body to be rendered…//             .click('button[type=submit]')…//             'required')
